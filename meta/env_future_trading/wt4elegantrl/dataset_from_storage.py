...
for period, name in {
    ...
    for code in securities:
        ...
        df = dt.get_bars(
            code, period=period, fromTime=201601011600, endTime=202110131600
        ).to_pandas()
        reader.set_bars(df)
        helper.trans_bars(
            barFile="%s/%s.dsb" % (path, code.replace(".HOT", "_HOT")),
            getter=reader.get_bars,
            count=len(reader),
            period="d" if period == "d1" else period,
        )
        # break
    # break
