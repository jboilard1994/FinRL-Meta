import numpy as np
import talib as ta
from reprocess import MAXMIN
from reprocess import REPROCESS
from reprocess import ZFILTER
...
class Feature:
    ...
    def __init__(
        self, code: str, period: str, roll: int, assets: float = 1000000
    ) -> None:
    ...
    def _subscribe_(self, period: str, count: int = 1):
        self.__subscribies__[period] = max(
            self.__subscribies__.get(period, 0), count + self._roll_
        )
        ...
        for code in self.__securities__:
            ...
            for period, count in self.__subscribies__.items():
                context.stra_get_bars(
                    ...
                    period=period,
                    count=count,
                    isMain=(code == self.__main__[0] and period == self.__main__[1]),
                )

    def _callback_(
        self, space: int, period: str, callback, reprocess: REPROCESS, **kwargs
    ):
    ...
    def observation(self) -> dict:
        ...
        self.__shape__ = (
            len(self.securities),
            sum(c[0] for v in self.__cb__.values() for c in v.values()) * self._roll_
            + 4,
        )
                ...
                for period, v in self.__cb__.items():  # 处理每一个周期
                    for space, callback, p, args in v.values():  # 处理每一个特征
                        ...
                        for feature in features:  # 处理每一个返回值
                            ...
                            obs[i][n : n + self._roll_] = p.calculate(feature)[
                                -self._roll_ :
                            ]
...
class Indicator(Feature):
        ...
        def roc(context: CtaContext, code: str, period: str, args: dict):
            ...
            return np.diff(
                context.stra_get_bars(
                    stdCode=code,
                    period=period,
                    count=self.__subscribies__[period],
                ).closes
            )
    ...
    def trange(self, period: str, reprocess: REPROCESS = MAXMIN):
        def trange(context: CtaContext, code: str, period: str, args: dict):
            ...
            return (
                ta.TRANGE(high=bars.highs, low=bars.lows, close=bars.closes)
                / bars.closes
            )
    ...
    def rsi(
        ...
        def rsi(context: CtaContext, code: str, period: str, args: dict):
            ...
            return (
                ta.RSI(bars.closes, args["fastperiod"]),
                ta.RSI(bars.closes, args["midperiod"]),
                ta.RSI(bars.closes, args["slowperiod"]),
            )
            return (
                ta.RSI(bars.closes, args["fastperiod"]) / 100,
                ta.RSI(bars.closes, args["midperiod"]) / 100,
                ta.RSI(bars.closes, args["slowperiod"]) / 100,
            )
    ...
    def dx(self, period: str, timeperiod=14, reprocess: REPROCESS = MAXMIN):
        def dx(context: CtaContext, code: str, period: str, args: dict):
            ...
            return (
                ta.DX(high=bars.highs, low=bars.lows, close=bars.closes, **args) / 100
            )
    ...
    def obv(self, period: str, reprocess: REPROCESS = MAXMIN):
        ...
        self._subscribe_(period=period, count=10 + reprocess.n())
        self._callback_(space=1, period=period, callback=obv, reprocess=reprocess)
