...
class Calculate:
    ...
    def get_annual_return(self):
        annual_return = (
            0
            if self.trade == 0
            else ((1 + self.ret.sum()) ** (self.period / self.trade) - 1)
        )
...
def continue_trading_analysis(data, x_value) -> dict:
    ...
    capital = 500000
    con_win_profit = (
        data["profit"].loc[con_win_p_end - win_time + 1 : con_win_p_end]
    ).sum()
    con_lose_loss = (
        data["profit"].loc[con_loss_p_end - loss_time + 1 : con_loss_p_end]
    ).sum()
...
def extreme_trading(data, time_of_std=1):
    ...
    # 极端交易数量
    extreme_result = data[
        data["profit"].apply(lambda x: x > sin_profit_plstd or x < sin_profit_mistd)
    ]
...
def performance_summary(input_data, input_data1, capital=500000, rf=0.00, period=240):
    ...
    input_data1["adjust_profit"] = (
        (input_data1["profit"] - input_data1["transaction_fee"])
    ...
    total_adjust_profit = (
        0 if len(adjust_profit) == 0 else adjust_profit["adjust_profit"].sum()
    )
    # 调整毛损
    adjust_loss = input_data1[input_data1["adjust_profit"].apply((lambda x: x < 0))]
    total_adjust_loss = (
        0 if len(adjust_loss) == 0 else adjust_loss["adjust_profit"].sum()
    )
    # 调整净利
    ...
    # 调整盈利因子
    adjust_profit_factor = (
        0 if total_adjust_loss == 0 else np.abs(total_adjust_profit / total_adjust_loss)
    )
    # 最大持有合约数量
    max_holding_number = 1
    # 已付手续费
    paid_trading_fee = (
        input_data1["transaction_fee"].sum() if len(input_data1) > 0 else 0
    )
    # 单笔最大亏损
    single_loss = input_data[input_data["profit"].apply(lambda x: x < 0)]
    single_largest_loss = (
        0 if len(single_loss) == 0 else abs(single_loss["profit"].min())
    )
    # 平仓交易最大亏损
...
def do_trading_analyze(df_closes, df_funds):
    ...
    winloseratio = (
        abs(avgprof_win / avgprof_lose) if avgprof_lose != 0 else "N/A"
    )  # 单次盈亏均值比
    ...
    avgtrd_hold_bar = (
        0
        if totaltimes == 0
        else ((df_closes["closebarno"] - df_closes["openbarno"]).sum()) / totaltimes
    )
    # 平均空仓K线根数
    ...
    winempty_avgholdbar = (
        0 if len(df_wins) == 0 else win_holdbar_situ.sum() / len(df_wins)
    )
    # 两笔亏损交易之间的偶平均空仓K线根数
    loss_holdbar_situ = (
        df_loses["openbarno"].shift(-1) - df_loses["closebarno"]
    ).dropna()
    lossempty_avgholdbar = (
        0 if len(df_loses) == 0 else loss_holdbar_situ.sum() / len(df_loses)
    )
...
def trading_analyze(workbook: Workbook, df_closes, df_funds, capital=500000):
    ...
    value_format = workbook.add_format(
        {"align": "right", "valign": "vcenter"}  # 水平居中  # 垂直居中
    )
    ...
    df_closes["opentime"] = df_closes["opentime"].apply(
        lambda x: parse(str(int(x / 10000)))
    )
    res = stat_closes_by_day(df_closes.copy(), capital)
    worksheet.write_row("A1", ["日度绩效分析"], title_format)
    worksheet.write_row(
        "A3",
        ["期间", "盈利(¤)", "盈利(%)", "毛利", "毛损", "交易次数", "胜率(%)"],
        index_format,
    )
...
def strategy_analyze(
    workbook: Workbook, df_closes, df_trades, capital, rf=0.0, period=240
):
    """
    策略分析
    """
    ...
    # 截取开仓明细
    data1_open = df_trades[
        df_trades["action"].apply(lambda x: "OPEN" in x)
    ].reset_index()
    data1_open = data1_open.drop(columns=["index"])
    # 截取平仓明细
    data1_close = df_trades[
        df_trades["action"].apply(lambda x: "CLOSE" in x)
    ].reset_index()
    data1_close = data1_close.drop(columns=["index"])

    # 将平仓明细字段重命名，并跟开仓明细合并成一个大表
    data1_close = data1_close.rename(
        columns={
            "code": "code_1",
            "time": "time_1",
            "direct": "direct_1",
    ...
    data_long = df_closes[
        df_closes["direct"].apply(lambda x: "LONG" in x)
    ].reset_index()
    after_merge_long = after_merge[
        after_merge["direct"].apply(lambda x: "LONG" in x)
    ].reset_index()
    data_short = df_closes[
        df_closes["direct"].apply(lambda x: "SHORT" in x)
    ].reset_index()
    after_merge_short = after_merge[
        after_merge["direct"].apply(lambda x: "SHORT" in x)
    ].reset_index()

    # 全部平仓明细进行绩效分析
    result1 = performance_summary(
        df_closes, after_merge, capital=capital, rf=rf, period=period
    )
    # 做多平仓明细进行绩效分析
    result1_2 = performance_summary(
        data_long, after_merge_long, capital=capital, rf=rf, period=period
    )
    # 做空平仓明细进行绩效分析
    result1_3 = performance_summary(
        data_short, after_merge_short, capital=capital, rf=rf, period=period
    )
    # 绩效比率计算
    result2 = ratio_calculate(
        df_closes, after_merge, capital=capital, rf=rf, period=period
    )
    # 时间分析
...
def output_closes(workbook: Workbook, df_closes: df, capital=500000):
    ...
    df_closes["entrytime"] = df_closes["opentime"].apply(
        lambda x: datetime.strptime(str(x), "%Y%m%d%H%M")
    )
    df_closes["exittime"] = df_closes["closetime"].apply(
        lambda x: datetime.strptime(str(x), "%Y%m%d%H%M")
    )
    ...
    ay = df_closes["entertag"].apply(
        lambda x: x if type(x) == str else "" if math.isnan(x) else x
    )
...
def summary_analyze(df_funds: df, capital=5000000, rf=0, period=240) -> dict:
    ...
    ar = (
        math.pow(ayNetVals.iloc[-1], annual_days / days) - 1
    )  # 年化收益率=总收益率^(年交易日天数/统计天数)
    ayDailyReturn = ayBal / ayPreBal - 1  # 每日收益率
    delta = fmtNAN(
        ayDailyReturn.std(axis=0) * math.pow(annual_days, 0.5), 0
    )  # 年化标准差=每日收益率标准差*根号下(年交易日天数)
    down_delta = fmtNAN(
        ayDailyReturn[ayDailyReturn < 0].std(axis=0) * math.pow(annual_days, 0.5),
        0,
    )  # 下行标准差=每日亏损收益率标准差*根号下(年交易日天数)
...
class WtBtAnalyst:
    ...
    def run_simple(self):
        ...
        for sname in self.__strategies__:
            ...
            sumObj = summary_analyze(
                df_funds, capital=init_capital, rf=rf, period=annual_days
            )
            sumObj["name"] = sname
            f = open(filename, "w")
