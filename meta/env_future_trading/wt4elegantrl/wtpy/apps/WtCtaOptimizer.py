...
class WtCtaOptimizer:
    ...
    def add_mutable_param(self, name: str, start_val, end_val, step_val, ndigits=1):
        ...
        @ndigits    小数位
        """
        self.mutable_params[name] = ParamInfo(
            name=name,
            start_val=start_val,
            end_val=end_val,
            step_val=step_val,
    ...
    def __execute_task__(self, params: dict):
        ...
        engine.configBTStorage(
            mode=self.env_params["storage_type"],
        ...
        if self.cpp_stra_module is not None:
            params.pop("name")
            engine.setExternalCtaStrategy(
                name, self.cpp_stra_module, self.cpp_stra_type, params
            )
        else:
    ...
    def go(
        self,
        interval: float = 0.2,
        out_marker_file: str = "strategies.json",
