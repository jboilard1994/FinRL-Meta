...
class WtCacheMonExchg(WtCacheMon):
    ...
    def getCffexData(curDT: datetime.datetime) -> dict:
        ...
        dtNum = int(dtStr)
        path = "http://www.cffex.com.cn/fzjy/mrhq/%d/%02d/index.xml" % (
            dtNum / 100,
            dtNum % 100,
        )
        ...
        for day in days:
            pid = day.getElementsByTagName("productid")[0].firstChild.data.strip()
            ...
            item.pid = pid
            item.hold = float(
                day.getElementsByTagName("openinterest")[0].firstChild.data
            )
            item.close = float(
                day.getElementsByTagName("closeprice")[0].firstChild.data
            )
            item.volume = int(day.getElementsByTagName("volume")[0].firstChild.data)
...
class WtMailNotifier:
    ...
    def add_receiver(self, name: str, addr: str):
        ...
        @addr   收件人邮箱地址
        """
        self.receivers.append({"name": name, "addr": addr})

    def notify(
        self,
        hot_changes: dict,
        sec_changes: dict,
        nextDT: datetime.datetime,
        hotFile: str,
        hotMap: str,
        secFile: str,
        secMap: str,
        ...
        xlspart.add_header(
            "Content-Disposition",
            "attachment",
            filename=os.path.basename(hotFile),
        )
        ...
        xlspart.add_header(
            "Content-Disposition",
            "attachment",
            filename=os.path.basename(hotMap),
        )
        msg_mp.attach(xlspart)
        ...
        xlspart.add_header(
            "Content-Disposition",
            "attachment",
            filename=os.path.basename(secFile),
        )
        msg_mp.attach(xlspart)
        ...
        xlspart.add_header(
            "Content-Disposition",
            "attachment",
            filename=os.path.basename(secMap),
        )
        msg_mp.attach(xlspart)
...
class WtHotPicker:
    ...
    def execute_rebuild(
        ...
        while curDate <= endDate:
            ...
            for exchg in exchanges:
                ...
                hotRules, secRules = self.pick_exchg_hots(
                    exchg, curDate, curDate, alg=alg
                )

                if len(hotRules.keys()) > 0:
                    hasChange, total_hots = self.merge_switch_list(
                        total_hots, exchg, hotRules
                    )
                ...
                if len(secRules.keys()) > 0:
                    hasChange, total_secs = self.merge_switch_list(
                        total_secs, exchg, secRules
                    )
    ...
    def execute_increment(
        ...
        if lastDate >= endDate.strftime("%Y%m%d"):
            ...
            beginDT = datetime.datetime.strptime(
                lastDate, "%Y%m%d"
