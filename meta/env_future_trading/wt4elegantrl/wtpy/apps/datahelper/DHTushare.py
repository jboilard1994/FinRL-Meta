...
class DHTushare(BaseDataHelper):
    ...
    def auth(self, **kwargs):
        ...
        self.isAuthed = True
        print(
            "Tushare has been authorized, use_pro is %s."
            % ("enabled" if self.use_pro else "disabled")
        )
    ...
    def dmpCodeListToFile(
        ...
        if hasStock:
            ...
            df_stocks = self.api.stock_basic(
                exchange="",
                list_status="L",
    ...
    def dmpAdjFactorsToFile(self, codes: list, filename: str):
        ...
        for stdCode in codes:
            ...
            for idx, row in df_factors.iterrows():
                ...
                items.append({"date": int(date), "factor": float(factor)})
    ...
    def __dmp_bars_to_file_from_pro__(
        self,
        folder: str,
        codes: list,
        ...
        for stdCode in codes:
            ...
            code = stdCode[-6:]
            asset_type = "E"
            if (exchg == "SSE" and code[0] == "0") | (
                exchg == "SZSE" and code[:3] == "399"
            ):
                asset_type = "I"
    ...
    def __dmp_bars_to_file_from_old__(
        self,
        folder: str,
        codes: list,
        start_date: datetime = None,
    ...
    def dmpBarsToFile(
        ...
        if self.use_pro:
            ...
            self.__dmp_bars_to_file_from_old__(
                folder=folder,
                codes=codes,
                start_date=start_date,
                end_date=end_date,
    ...
    def dmpAdjFactorsToDB(self, dbHelper: DBHelper, codes: list):
        ...
        for stdCode in codes:
            ...
            for idx, row in df_factors.iterrows():
                date = row["trade_date"]
                factor = row["adj_factor"]
                items.append({"date": int(date), "factor": factor})
    ...
    def __dmp_bars_to_db_from_pro__(
        self,
        dbHelper: DBHelper,
        codes: list,
        start_date: datetime = None,
    ...
    def __dmp_bars_to_db_from_old__(
        self,
        dbHelper: DBHelper,
        codes: list,
        start_date: datetime = None,
    ...
    def dmpBarsToDB(
        ...
        if self.use_pro:
            self.__dmp_bars_to_db_from_pro__(
                dbHelper=dbHelper,
                codes=codes,
                start_date=start_date,
                end_date=end_date,
            ...
            self.__dmp_bars_to_db_from_old__(
                dbHelper=dbHelper,
                codes=codes,
                start_date=start_date,
                end_date=end_date,
