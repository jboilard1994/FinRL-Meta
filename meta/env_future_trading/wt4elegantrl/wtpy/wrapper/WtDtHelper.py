...
class WtDataHelper:
    ...
    def __init__(self):
        ...
        self.api.resample_bars.argtypes = [
            c_char_p,
            CB_DTHELPER_BAR,
            CB_DTHELPER_COUNT,
            c_uint64,
    ...
    def dump_bars(self, binFolder: str, csvFolder: str, strFilter: str = ""):
        ...
        """
        self.api.dump_bars(
            bytes(binFolder, encoding="utf8"),
            bytes(csvFolder, encoding="utf8"),
            bytes(strFilter, encoding="utf8"),
    ...
    def read_dsb_ticks(self, tickFile: str) -> TickList:
        ...
        """
        tick_cache = TickList()
        if 0 == self.api.read_dsb_ticks(
            bytes(tickFile, encoding="utf8"),
            CB_DTHELPER_TICK(tick_cache.on_read_tick),
            CB_DTHELPER_COUNT(tick_cache.on_data_count),
            self.cb_dthelper_log,
        ):
            return None
        else:
    ...
    def read_dsb_bars(self, barFile: str) -> BarList:
        ...
        """
        bar_cache = BarList()
        if 0 == self.api.read_dsb_bars(
            bytes(barFile, encoding="utf8"),
            CB_DTHELPER_BAR(bar_cache.on_read_bar),
    ...
    def trans_ticks(self, tickFile: str, getter, count: int) -> bool:
        ...
        return self.api.trans_ticks(
            bytes(tickFile, encoding="utf8"), cb, count, self.cb_dthelper_log
        )
