...
from wtpy.WtCoreDefs import CB_ENGINE_EVENT
from wtpy.WtCoreDefs import CB_EXECUTER_CMD
from wtpy.WtCoreDefs import CB_EXECUTER_INIT
from wtpy.WtCoreDefs import CB_HFTSTRA_CHNL_EVT
from wtpy.WtCoreDefs import CB_HFTSTRA_ENTRUST
from wtpy.WtCoreDefs import CB_HFTSTRA_GET_ORDDTL
from wtpy.WtCoreDefs import CB_HFTSTRA_GET_ORDQUE
from wtpy.WtCoreDefs import CB_HFTSTRA_GET_TRANS
from wtpy.WtCoreDefs import CB_HFTSTRA_ORD
from wtpy.WtCoreDefs import CB_HFTSTRA_ORDDTL
from wtpy.WtCoreDefs import CB_HFTSTRA_ORDQUE
from wtpy.WtCoreDefs import CB_HFTSTRA_TRANS
from wtpy.WtCoreDefs import CB_HFTSTRA_TRD
from wtpy.WtCoreDefs import CB_PARSER_EVENT
from wtpy.WtCoreDefs import CB_PARSER_SUBCMD
from wtpy.WtCoreDefs import CB_SESSION_EVENT
from wtpy.WtCoreDefs import CB_STRATEGY_BAR
from wtpy.WtCoreDefs import CB_STRATEGY_CALC
from wtpy.WtCoreDefs import CB_STRATEGY_GET_BAR
from wtpy.WtCoreDefs import CB_STRATEGY_GET_POSITION
from wtpy.WtCoreDefs import CB_STRATEGY_GET_TICK
from wtpy.WtCoreDefs import CB_STRATEGY_INIT
from wtpy.WtCoreDefs import CB_STRATEGY_TICK
from wtpy.WtCoreDefs import CHNL_EVENT_LOST
from wtpy.WtCoreDefs import CHNL_EVENT_READY
from wtpy.WtCoreDefs import EVENT_ENGINE_INIT
from wtpy.WtCoreDefs import EVENT_ENGINE_SCHDL
from wtpy.WtCoreDefs import EVENT_PARSER_CONNECT
from wtpy.WtCoreDefs import EVENT_PARSER_DISCONNECT
from wtpy.WtCoreDefs import EVENT_PARSER_INIT
from wtpy.WtCoreDefs import EVENT_PARSER_RELEASE
from wtpy.WtCoreDefs import EVENT_SESSION_BEGIN
from wtpy.WtCoreDefs import EVENT_SESSION_END
from wtpy.WtCoreDefs import WTSBarStruct
from wtpy.WtCoreDefs import WTSOrdDtlStruct
from wtpy.WtCoreDefs import WTSOrdQueStruct
from wtpy.WtCoreDefs import WTSTickStruct
from wtpy.WtCoreDefs import WTSTransStruct
...
class WtWrapper:
    ...
    def __init__(self, engine):
        ...
        self.api.cta_enter_short.argtypes = [
            c_ulong,
            c_char_p,
            c_double,
            c_char_p,
        ...
        self.api.cta_exit_long.argtypes = [
            c_ulong,
            c_char_p,
            c_double,
            c_char_p,
        ...
        self.api.cta_exit_short.argtypes = [
            c_ulong,
            c_char_p,
            c_double,
            c_char_p,
        ...
        self.api.cta_set_position.argtypes = [
            c_ulong,
            c_char_p,
            c_double,
            c_char_p,
        ...
        self.api.sel_set_position.argtypes = [
            c_ulong,
            c_char_p,
            c_double,
            c_char_p,
    ...
    def on_stra_bar(
        self, id: int, stdCode: str, period: str, newBar: POINTER(WTSBarStruct)
    ):
    ...
    def on_stra_get_tick(
        self,
        id: int,
        stdCode: str,
        curTick: POINTER(WTSTickStruct),
        count: int,
        isLast: bool,
    ):
        """
    ...
    def on_hftstra_order_queue(
        self, id: int, stdCode: str, newOrdQue: POINTER(WTSOrdQueStruct)
    ):
        stdCode = bytes.decode(stdCode)
        engine = self._engine
        ctx = engine.get_context(id)
    ...
    def on_hftstra_get_order_queue(
        self,
        id: int,
        stdCode: str,
        newOrdQue: POINTER(WTSOrdQueStruct),
        count: int,
        isLast: bool,
    ):
        engine = self._engine
        ...
        for i in range(count):
            ...
            curOrdQue = dict()
            curOrdQue["time"] = (
                realOrdQue.action_date * 1000000000 + realOrdQue.action_time
            )
    ...
    def on_hftstra_order_detail(
        self, id: int, stdCode: str, newOrdDtl: POINTER(WTSOrdDtlStruct)
    ):
        engine = self._engine
        ctx = engine.get_context(id)
    ...
    def on_hftstra_get_order_detail(
        self,
        id: int,
        stdCode: str,
        newOrdDtl: POINTER(WTSOrdDtlStruct),
        count: int,
        isLast: bool,
    ):
        engine = self._engine
        ...
        for i in range(count):
            ...
            curOrdDtl = dict()
            curOrdDtl["time"] = (
                realOrdDtl.action_date * 1000000000 + realOrdDtl.action_time
            )
    ...
    def on_hftstra_transaction(
        self, id: int, stdCode: str, newTrans: POINTER(WTSTransStruct)
    ):
        engine = self._engine
        ctx = engine.get_context(id)
    ...
    def on_hftstra_get_transaction(
        self,
        d: int,
        stdCode: str,
        newTrans: POINTER(WTSTransStruct),
        count: int,
        isLast: bool,
    ):
        engine = self._engine
        ...
        for i in range(count):
            ...
            curTrans = dict()
            curTrans["time"] = (
                realTrans.action_date * 1000000000 + realTrans.action_time
            )
    ...
    def write_log(self, level, message: str, catName: str = ""):
        self.api.write_log(
            level,
            bytes(message, encoding="utf8").decode("utf-8").encode("gbk"),
    ...
    def push_quote_from_exetended_parser(
        self, id: str, newTick: POINTER(WTSTickStruct), bNeedSlice: bool = True
    ):
        return self.api.parser_push_quote(
            bytes(id, encoding="utf8"), newTick, bNeedSlice
        )
    ...
    def initialize_cta(
        ...
        try:
            ...
            self.api.init_porter(
                bytes(logCfg, encoding="utf8"),
                isFile,
                bytes(genDir, encoding="utf8"),
            )
    ...
    def initialize_hft(
        ...
        try:
            self.api.register_evt_callback(self.cb_engine_event)
            self.api.register_hft_callbacks(
                self.cb_stra_init,
                self.cb_stra_tick,
                self.cb_stra_bar,
                self.cb_hftstra_chnl_evt,
                self.cb_hftstra_order,
                self.cb_hftstra_trade,
                self.cb_hftstra_entrust,
                self.cb_hftstra_orddtl,
                self.cb_hftstra_ordque,
                self.cb_hftstra_trans,
                self.cb_session_event,
            )
            self.api.init_porter(
                bytes(logCfg, encoding="utf8"),
    ...
    def initialize_sel(
        ...
        try:
            self.api.register_evt_callback(self.cb_engine_event)
            self.api.register_sel_callbacks(
                self.cb_stra_init,
                self.cb_stra_tick,
                self.cb_stra_calc,
                self.cb_stra_bar,
    ...
    def cta_get_bars(
        self, id: int, stdCode: str, period: str, count: int, isMain: bool
    ):
        """
        读取K线\n
        @id         策略id\n
        ...
        @isMain     是否主K线
        """
        return self.api.cta_get_bars(
            id,
            bytes(stdCode, encoding="utf8"),
            bytes(period, encoding="utf8"),
            count,
            isMain,
    ...
    def cta_get_all_position(self, id: int):
        ...
        @id     策略id
        """
        return self.api.cta_get_all_position(
            id, CB_STRATEGY_GET_POSITION(self.on_stra_get_position)
        )
    ...
    def cta_get_position(self, id: int, stdCode: str, usertag: str = ""):
        ...
        """
        return self.api.cta_get_position(
            id,
            bytes(stdCode, encoding="utf8"),
            bytes(usertag, encoding="utf8"),
        )
    ...
    def cta_log_text(self, id: int, message: str):
        ...
        @message    日志内容
        """
        self.api.cta_log_text(
            id, bytes(message, encoding="utf8").decode("utf-8").encode("gbk")
        )
    ...
    def cta_get_detail_entertime(self, id: int, stdCode: str, usertag: str) -> int:
        ...
        @return     进场时间，格式如201907260932
        """
        return self.api.cta_get_detail_entertime(
            id,
            bytes(stdCode, encoding="utf8"),
            bytes(usertag, encoding="utf8"),
        )
    ...
    def cta_get_detail_cost(self, id: int, stdCode: str, usertag: str) -> float:
        ...
        @return     开仓价
        """
        return self.api.cta_get_detail_cost(
            id,
            bytes(stdCode, encoding="utf8"),
            bytes(usertag, encoding="utf8"),
        )
    ...
    def cta_get_detail_profit(self, id: int, stdCode: str, usertag: str, flag: int):
        ...
        @return     盈亏
        """
        return self.api.cta_get_detail_profit(
            id,
            bytes(stdCode, encoding="utf8"),
            bytes(usertag, encoding="utf8"),
            flag,
    ...
    def cta_save_user_data(self, id: int, key: str, val: str):
        ...
        @val        数据值
        """
        self.api.cta_save_userdata(
            id, bytes(key, encoding="utf8"), bytes(val, encoding="utf8")
        )
    ...
    def cta_load_user_data(self, id: int, key: str, defVal: str = ""):
        ...
        @defVal     默认值
        """
        ret = self.api.cta_load_userdata(
            id, bytes(key, encoding="utf8"), bytes(defVal, encoding="utf8")
        )
    ...
    def sel_load_user_data(self, id: int, key: str, defVal: str = ""):
        ...
        @defVal     默认值
        """
        ret = self.api.sel_load_userdata(
            id, bytes(key, encoding="utf8"), bytes(defVal, encoding="utf8")
        )
        return bytes.decode(ret)
    ...
    def sel_get_all_position(self, id: int):
        ...
        @id     策略id
        """
        return self.api.sel_get_all_position(
            id, CB_STRATEGY_GET_POSITION(self.on_stra_get_position)
        )
    ...
    def sel_get_position(self, id: int, stdCode: str, usertag: str = ""):
        ...
        @return 指定合约的持仓手数，正为多，负为空
        """
        return self.api.sel_get_position(
            id,
            bytes(stdCode, encoding="utf8"),
            bytes(usertag, encoding="utf8"),
        )
    ...
    def hft_get_ordque(self, id: int, stdCode: str, count: int):
        ...
        @count     条数\n
        """
        return self.api.hft_get_ordque(
            id,
            bytes(stdCode, encoding="utf8"),
            count,
    ...
    def hft_get_orddtl(self, id: int, stdCode: str, count: int):
        ...
        @count     条数\n
        """
        return self.api.hft_get_orddtl(
            id,
            bytes(stdCode, encoding="utf8"),
            count,
    ...
    def hft_load_user_data(self, id: int, key: str, defVal: str = ""):
        ...
        @defVal     默认值
        """
        ret = self.api.hft_load_userdata(
            id, bytes(key, encoding="utf8"), bytes(defVal, encoding="utf8")
        )
        return bytes.decode(ret)
    ...
    def create_hft_context(self, name: str, trader: str, agent: bool) -> int:
        ...
        @return    系统内策略ID
        """
        return self.api.create_hft_context(
            bytes(name, encoding="utf8"), bytes(trader, encoding="utf8"), agent
        )

    def create_sel_context(
        self,
        name: str,
        date: int,
        time: int,
        ...
        @return    系统内策略ID
        """
        return self.api.create_sel_context(
            bytes(name, encoding="utf8"),
            date,
            time,
            bytes(period, encoding="utf8"),
            bytes(trdtpl, encoding="utf8"),
